...
class Uploader implements RemoteUploaderInterface
{
    ...
    public function chunkDownloadCloudFileToFolder(string $fileId, int $fileSize, int $chunkStart): int
    {
        ...
        if ($chunkStart < $fileSize) {
            ...
            $response = $this->runRemoteRequest(self::DROPBOX_API_FILE_DOWNLOAD_URL, $args, false);
            if (!$response) {
                return 0;
            }
     ...
     */
    protected function runRemoteRequest(string $url, array $args = [], bool $decodeBody = true)
    {
        $defaults = [
            'timeout'     => 120,
            'httpversion' => '1.0',
